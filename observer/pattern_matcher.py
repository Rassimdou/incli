...
class PatternMatcher:
    ...
    def match_errors(self, text: str) -> List[Tuple[str, float]]:
        ...
        Returns:
            List of (error_type, confidence) tuples
        """
        matches = []
        ...
        for error_type, error_data in self.error_patterns.items():
            for pattern in error_data["patterns"]:
                if re.search(pattern, text_lower, re.IGNORECASE):
                    ...
                    break  # don't match patterns for same type 

        return matches 
        
    def match_waf(self, headers: Dict[str, str], body: str, status_code: int) -> List[Tuple[str, float, List[str]]]:
        """
    ...
    def detect_server(self, headers: Dict[str, str]) -> Optional[Tuple[str, Optional[str], float]]:
        """
    ...
    def detect_language(self, headers: Dict[str, str], body: str) -> List[Tuple[str, Optional[str], float, str]]:
        """
    ...
    def check_execution(self, body: str) -> Tuple[bool, str, float]:
        ...
        # If response is short and doesn't contain html/source, likely executed
        if len(body) < 10000 and "<html" not in body.lower() and "<?php" not in body:
            if body.strip():  # Not empty 
                return (True, "Non-HTML response with no source code visible", 0.6)
                
        return (False, "No execution indicators", 0.5)
            
    def extract_upload_path(self, response_text: str, headers: Dict[str, str]) -> Optional[str]:
        """
    ...
    def analyze_blocking_behavior(self, baseline_response, injected_response) -> Dict[str, any]:
        """
        ...
        waf_matches = self.match_waf(
            dict(injected_response.headers),
